...
from AllMethods.Group_08.checkEqual.src.checkEqual import checkEqual
...
def _get_outputs(all_inputs):
    ...
    try:
        inputs_outputs['td_output'] = checkEqual(all_inputs['td'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['td_output'] = 'error'
    try:
        inputs_outputs['ttd_output_mr_per'] = checkEqual(all_inputs['ttd_mr_per'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_per'] = 'error'
        
    try:
        inputs_outputs['ttd_output_mr_add'] = checkEqual(all_inputs['ttd_mr_add'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_add'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_mul'] = checkEqual(all_inputs['ttd_mr_mul'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_mul'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_inv'] = checkEqual(all_inputs['ttd_mr_inv'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_inv'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_inc'] = checkEqual(all_inputs['ttd_mr_inc'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_inc'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_exc'] = checkEqual(all_inputs['ttd_mr_exc'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
