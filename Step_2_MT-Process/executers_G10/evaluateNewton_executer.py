...
from AllMethods.Group_10.evaluateNewton.src.evaluateNewton import evaluateNewton
...
def _get_outputs(all_inputs):
    ...
    try:
        inputs_outputs['td_output'] = evaluateNewton(all_inputs['td'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['td_output'] = 'error'
    try:
        inputs_outputs['ttd_output_mr_per'] = evaluateNewton(all_inputs['ttd_mr_per'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_per'] = 'error'
        
    try:
        inputs_outputs['ttd_output_mr_add'] = evaluateNewton(all_inputs['ttd_mr_add'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_add'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_mul'] = evaluateNewton(all_inputs['ttd_mr_mul'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_mul'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_inv'] = evaluateNewton(all_inputs['ttd_mr_inv'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_inv'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_inc'] = evaluateNewton(all_inputs['ttd_mr_inc'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_inc'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_exc'] = evaluateNewton(all_inputs['ttd_mr_exc'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
