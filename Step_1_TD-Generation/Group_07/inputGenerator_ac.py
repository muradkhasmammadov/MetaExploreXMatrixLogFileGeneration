...
def compute_moment3_and_variance(data):
    try:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=RuntimeWarning)
            # Calculating the third moment (skewness)
            moment3 = skew(data, bias=False)  # Setting bias=False for sample skewness

            # Calculating the sample variance
            sample_variance = np.var(data, ddof=1)

        return moment3, sample_variance
...
def fuzzer(input_type):
    ...
    if input_type == 'int':
        ...
        moment3, sample_variance = compute_moment3_and_variance(a)
